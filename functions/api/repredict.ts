// functions/api/repredict.ts - Force repredict with multi-tier fallback and KV refresh
...
async function fetchLotteryRecords(): Promise<DrawRecord[]> {
  try {
    ...
    console.warn('Fetch lottery records error in repredict, using fallback history:', err);
  }
...
export const onRequestPost = async (context: { request: Request; env: Env }) => {
  ...
  if (records.length === 0) {
    return new Response(
      JSON.stringify({ status: 'error', message: '无法获取大盘数据' }),
  ...
  // Purge old KV cache for current and previous periods
  if (kv && typeof kv.delete === 'function') {
    try {
      ...
      console.warn('KV repredict delete error:', e);
    }
  }
...
必须返回符合以下格式的 JSON：
{
  ...
  "reasoning": {
    ...
    for (const modelName of FALLBACK_MODELS) {
      try {
        console.log(`[CF Pages Repredict] Invoking ${modelName}...`);
        ...
        const msg = `${modelName} 超时或异常: ${err.message || '未知'}`;
        console.warn(`[CF Pages Repredict Fallback] ${msg}`);
  ...
  return new Response(
    JSON.stringify({
      ...
      message: prediction.isAIPowered 
        ? `推演完成 (${prediction.modelUsed}${prediction.isFallback ? ' · 平滑回退' : ''})` 
        : '所有模型调用受限，已切换至高精度数理对冲保底',
      prediction,
    }),
